...
namespace Mathos.Parser {
    ...
    public class MathParser {
        ...
        public MathParser(
            ...
            if(loadPreDefinedOperators) {
                Operators = new Dictionary<string, Func<double, double, double>> {
                    ...
                    ["&"] = (a, b) => Math.Abs(a - 1) < 0.00000001 && Math.Abs(b - 1) < 0.00000001 ? 1 : 0,
                    ["|"] = (a, b) => Math.Abs(a - 1) < 0.00000001 || Math.Abs(b - 1) < 0.00000001 ? 1 : 0
            ...
            if(loadPreDefinedFunctions) {
                LocalFunctions = new Dictionary<string, Func<double[], double>> {
                    ...
                    ["round"] = inputs => Math.Round(inputs[0], inputs.Length > 1 ? (int)inputs[1] : 0, MidpointRounding.AwayFromZero),
                    ["truncate"] = inputs => inputs[0] < 0 ? -Math.Floor(-inputs[0]) : Math.Floor(inputs[0]),

                    ["max"] = inputs => Math.Max(inputs[0], inputs[1]),
                    ["min"] = inputs => Math.Min(inputs[0], inputs[1]),
        ...
        private double BasicArithmeticalExpression(List<string> tokens) {
            ...
            switch(tokens.Count) {
                case 1:
                    if(!double.TryParse(tokens[0], NumberStyles.Float, CultureInfo, out token0)) {
                ...
                case 2:
                    ...
                    if(op == "-" || op == "+") {
                        ...
                        if(!double.TryParse(first + tokens[1], NumberStyles.Float, CultureInfo, out token1)) {
                    ...
                    if(!double.TryParse(tokens[1], NumberStyles.Float, CultureInfo, out token1)) {
            ...
            foreach(var op in Operators) {
                ...
                while((opPlace = tokens.IndexOf(op.Key)) != -1) {
                    ...
                    if(!double.TryParse(tokens[opPlace + 1], NumberStyles.Float, CultureInfo, out rhs)) {
                    ...
                    if(op.Key == "-" && opPlace == 0) {
                        ...
                        if(!double.TryParse(tokens[opPlace - 1], NumberStyles.Float, CultureInfo, out lhs)) {
                            throw new Exception("local variable " + tokens[opPlace - 1] + " is undefined");
                        }
